...
class Agent(AgentConfig):
    ...
    The agent finds a trajectory of steps (episode) and updates the values of
    the network to adjust the policy based on the advantages and probabilites
    of taking actions in each state. Another episode is taken with the new 
    network and the process is repeated until the problem is solved.
    Saves the running average of the episodes rewards in self.average_reward.
    """
    def train(self):
        episode = 0
        step = 0
        ...
        # A new episode
        while (not solved) and (len(reward_history) < 250):
            start_step = step
            episode += 1
            episode_length = 0
            ...
            while not solved:
                ...
                if terminal or (episode_length > 500):
                    ...
                    if len(reward_history) > 100 and sum(reward_history[-100:-1]) / 100 >= 500:
                        solved = True
        ...
        self.average_reward = avg_reward
        self.env.close()

    """
    Finds the ratio of the new and old probabilities and applies it to the PPO
    objective function. Optimises this function to update the network.
    """
    ...
    # final calculatations and updates when trajectory is completed
    def finish_path(self, length):
        ...
        # get advantage by taking the difference between the expected reward 
        # and actual reward
        ...
        advantages = []
        adv = 0.0
        # advantage takes into account future states and their advantages with
        # a discount factor
        for d in delta[::-1]:
